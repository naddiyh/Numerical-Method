from tabulate import tabulate  #Mengimport tabulate untuk menampilkan tabel
from math import e
...
# Loop utama untuk program, memberikan opsi untuk memilih metode penyelesaian dan menginput nilai.
while True:
    print("Masukkan nilai yang ingin digunakan:")
    try:
        xl = float(input("xl: ")) 
        xu = float(input("xu: "))  
        approxError = float(input("ɛa: ")) #approxError = Approxiamte Error
        print("\nMetode")
        print("1. Bisection")
        print("2. False-Position")
        menu = input("Pilih metode (angka): " )

        # Menjalankan metode Bisection Method
        if menu == '1':
            def bisection(xl, xu, approxError):
                print("Hasil Perhitungan Bisection Method")
        ...
        # Menjalankan metode False Position
        elif menu == "2":
            def FalsePosition(xl, xu , approxError):
                print("Hasil Perhitungan False-Position")
    ...
    # Menangkap kesalahan input, nilai harus berupa angka
    except ValueError:
